...
class Eval:
    ...
    def compute_vals(self, gold_chart, output_chart):
        """
        [compute_vals()] calculates the total number of correct predict
        """
        num_sent = len(output_chart)
        num_true_pos, n_out, n_gold = 0, 0, 0

        acc = lambda n, i, j, chart: n+1 if (chart[i, j] != 0) else n

        for i in range(num_sent):
            for j in range(len(output_chart[i])):
                n_gold = acc(n_gold, i, j, gold_chart)
                n_out = acc(n_out, i, j, output_chart)
                if gold_chart[i, j] != 0 and output_chart[i, j] != 0:
                    num_true_pos += 1
    ...
    def evaluate(self, output_charts, gold_charts):
        ...
        tot_outs = []
        tot_golds = []
        for i in range(len(output_charts)):
            correct, tot_out, tot_gold = self.compute_vals(output_charts[i], gold_charts[i])
            corrects.append(correct)
            tot_outs.append(tot_out)
            tot_golds.append(tot_gold)
